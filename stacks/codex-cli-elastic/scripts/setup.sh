...
#   3. agent — register the UserPromptSubmit characterization hook into
...
echo "[setup] 2/4 — local Codex session config (.codex/config.toml, [otel] telemetry)"
"$C/agents/codex-cli/scripts/render-config.sh" "$OTLP_ENDPOINT" "$STACK_DIR"

echo "[setup] 3/4 — UserPromptSubmit capture hook (.codex/hooks.json, characterization)"
