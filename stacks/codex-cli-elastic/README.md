...
> [`codex-cli-elastic-audit`](../codex-cli-elastic-audit/) stack.
...
**Naming.** `codex-cli` mirrors the `service.name: codex-cli` Codex CLI emits (as
`claude-code` does its own) and disambiguates from Codex cloud / the IDE extension / the
legacy Codex model. The actual emitted `service.name` is **`codex_cli_rs`**, so Codex
...
`stacks/codex-cli-elastic/.codex/` (gitignored) whose `config.toml` carries the `[otel]`
...
> in the sibling [`codex-cli-elastic-audit`](../codex-cli-elastic-audit/) stack
...
points it at its repo `components/agents/codex-cli/hooks/`) with
...
`requirements.toml` points `managed_dir` at this repo's `components/agents/codex-cli/hooks/`
...
The **APM UI** (<http://localhost:5601/app/apm>) also lists `codex-cli` as a service once
telemetry arrives.
...
| APM service | `codex-cli` | listed once telemetry arrives |
...
is the sibling [`codex-cli-elastic-audit`](../codex-cli-elastic-audit/) stack.) Authored
...
searches under `kibana/codex-cli/` — `data-views.ndjson`, `saved-searches.ndjson`), and the
generic appliers. The Codex CLI agent component (`../../components/agents/codex-cli/`) owns
