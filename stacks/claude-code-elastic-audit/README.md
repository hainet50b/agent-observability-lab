...
> [`claude-code-elastic`](../claude-code-elastic/): a direct hook → Elasticsearch write
...
`stacks/claude-code-elastic-audit/.claude/` (gitignored) carrying the audit config —
...
(`../../components/agents/claude-code/`) owns only agent-runtime config — the single
