...
#             capture-tool-call.sh directly would not catch a broken rendered hook
...
HOOK="$REPO_ROOT/components/agents/claude-code/hooks/capture-tool-call.sh"
...
# than invoking capture-tool-call.sh directly — so a broken rendered hook command is
