...
Collector variant is the separate [`claude-code-otelcol-elastic`](../claude-code-otelcol-elastic/)
stack.
...
`stacks/claude-code-elastic/`** and both telemetry and prompt auditing are on, with
...
`OTEL_LOG_RAW_API_BODIES` section of [`../../SPEC/claude-code-telemetry.md`](../../SPEC/claude-code-telemetry.md).
...
`kibana/claude-code/`), and the generic appliers that load them. The Claude Code agent
component (`../../components/agents/claude-code/`) owns only agent-runtime config — the
