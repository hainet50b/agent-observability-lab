...
foreach ($line in Get-Content -LiteralPath $Config) {
    ...
    $k, $v = $line -split '=', 2
    switch ($k.Trim()) {
        'telemetry.otel_collector.endpoint' {
            $OtlpEndpoint = $v.Trim()
        }
    }
}
if (-not $OtlpEndpoint) {
    [Console]::Error.WriteLine("FAIL: ${Config}: missing or empty key 'telemetry.otel_collector.endpoint'.")
    exit 2
}

# Optional secret overlay (gitignored): telemetry.otel_collector.api_key.
...
if (Test-Path -LiteralPath $LocalConfig -PathType Leaf) {
    foreach ($line in Get-Content -LiteralPath $LocalConfig) {
        ...
        $k, $v = $line -split '=', 2
        if ($k.Trim() -eq 'telemetry.otel_collector.api_key') {
            $OtlpApiKey = $v.Trim()
        }
    }
}
...
switch ($Scope) {
    'local' {
        ...
        $Target = $StackDir
        if ($Teardown) {
            & (Join-Path $ComponentsDir 'agents/claude-code/scripts/teardown-local.ps1') -TargetDir $Target -Endpoint $OtlpEndpoint
        }
        else {
            & (Join-Path $ComponentsDir 'agents/claude-code/scripts/setup-telemetry.ps1') -TargetDir $Target -OtlpEndpoint $OtlpEndpoint -ApiKey $OtlpApiKey
            & (Join-Path $ComponentsDir 'agents/claude-code/scripts/render-mcp.ps1') -TargetDir $Target -Endpoint $OtlpEndpoint
        }
    }
    'project' {
        if (-not $Target) {
    ...
    'managed' {
        if ($Teardown) {
            ...
            & (Join-Path $ComponentsDir 'agents/claude-code/scripts/teardown-managed.ps1') @TeardownArgs
        }
        else {
