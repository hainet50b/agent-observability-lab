...
| Plane | Keys | Notes |
| --- | --- | --- |
| **Control plane** — `../../components/backends/espalier.toml` | `[targets.local]` in-network endpoints, `[groups.claude-elastic-audit]` selection | run by `scripts/provision.{sh,ps1}` via the espalier container |
