...
`aol-kibana`) are the `elasticsearch` and `kibana` fragments of the Elastic backend
family — the same local Elasticsearch as the telemetry stack, **minus APM Server**
(this stack simply doesn't include that fragment). `scripts/setup.sh` runs the post-up
bootstrap in one shot — provisions the full Elastic asset set (the single `elastic`
espalier group: the Agent Audit data streams and their strict index templates, plus the
telemetry assets, which sit idle here without an APM Server), registers
...
The config is split by reader — the backend control plane in the shared espalier project
(`../../backends/elastic/espalier.toml`), the agent data plane at the stack root — and each
reader fails fast if its file is missing or a required key is empty rather than assuming
localhost:

| Plane | Keys | Notes |
| --- | --- | --- |
| **Control plane** — `../../backends/elastic/espalier.toml` | `[targets.local]` in-network endpoints, `[groups.elastic]` selection | run by `scripts/provision.{sh,ps1}` via the espalier container |
...
Elasticsearch MCP, the Agent Audit data views + saved searches, and **prompt / tool-I/O
sealing** (opt-in, off in lab mode): a capture stream set to `content = "encrypted"` has its
body sealed at the edge to an epoch recipient cert (null `text`, populated `encrypted_text`).
Select the epoch via the commented `[agent_audit.seal]` table in `agent-config.toml` and point
the CLI at the cert with `--seal-recipients ../../sealing/recipients` (this repo keeps the
recipients root at the top level) or `--seal-cert <pem>`; the cert CN must equal the epoch.
Key issuance and decryption are center-side utilities under the repo-root `sealing/` (see its
README). Nothing in this stack is currently deferred.

## Layout

```
claude-elastic-audit/
├─ docker-compose.yml                     # thin composition: `include:`s the elasticsearch + kibana fragments (no APM Server)
...
└─ scripts/
   ...
   ├─ provision.sh / .ps1                # delegates to ../../backends/elastic/provision (espalier apply --group elastic)
...
The Elastic backend family
(`../../backends/elastic/`) is self-contained: each service fragment
(`elasticsearch/`, `kibana/`, `apm-server/` — this stack includes only the first two)
owns its service definition, config, and the assets that service consumes (the Agent
Audit data-stream ILM / `@mappings` + `@lifecycle` component templates / index
templates under `elasticsearch/agent-audit/`; the Agent Audit data views and saved
searches under `kibana/agent-audit/`), and the espalier project plus
`provision.{sh,ps1}` sit beside the fragments. The agent-side content
(`../../agents/claude/`) owns only agent-runtime config — the single
audit-hook entry `hooks/agent-audit.{sh,ps1}` over its per-agent `hooks/lib/adapter.{sh,ps1}`
(the shared hook core lives once under `../../agents/shared/agent-audit/lib/`) and
