...
(`../../components/backends/espalier.toml`), the agent data plane at the stack root — and each
reader fails fast if its file is missing or a required key is empty rather than assuming
localhost:

| Plane | Keys | Notes |
| --- | --- | --- |
| **Control plane** — `../../components/backends/espalier.toml` | `[targets.stack]` in-network endpoints, `[groups.claude-elastic-audit]` selection | run by `scripts/provision.{sh,ps1}` via the espalier container |
...
└─ scripts/
   ...
   ├─ provision.sh / .ps1                # espalier apply --group claude-elastic-audit (docker, in-network)
