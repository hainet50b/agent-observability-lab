...
espalier group: the two Agent Audit data streams and their strict index templates, plus
the telemetry assets, which sit idle here without an APM Server), renders
...
`provision.{sh,ps1}` sit beside the fragments. The agent-side content (`../../agents/codex/`)
...
`../../agents/shared/agent-audit/lib/`) and the hook delivery-config template.
