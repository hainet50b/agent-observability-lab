...
local)
  ...
  if [ "$teardown" -eq 1 ]; then
    "$components_dir/agents/codex/scripts/teardown-local.sh" "$target" "$AUDIT_ES_URL"
  else
    "$components_dir/agents/codex/scripts/setup-audit.sh" "$target" "$AUDIT_ES_URL" "$AUDIT_API_KEY" \
      "$AUDIT_TIMEOUT_MS" "$AUDIT_UP_ENABLED" "$AUDIT_UP_CONTENT" "$AUDIT_TC_ENABLED" "$AUDIT_TC_CONTENT" \
      "$AUDIT_ES_URL" "$SEAL_SRC" "$SEAL_KEY_ID"
    "$components_dir/agents/codex/scripts/render-mcp.sh" "$target" "$AUDIT_ES_URL"
    "$components_dir/agents/codex/scripts/link-auth.sh" "$target/.codex" "$AUDIT_ES_URL"
  fi
  ;;
project)
  [ -n "$target" ] || {
...
managed)
  ...
  if [ "$teardown" -eq 1 ]; then
    "$components_dir/agents/codex/scripts/teardown-managed.sh" --with-hooks
  else
    "$components_dir/agents/codex/scripts/setup-managed.sh" --with-hooks --es-url "$AUDIT_ES_URL" --es-api-key "$AUDIT_API_KEY" \
