...
switch ($Scope) {
    'local' {
        ...
        $Target = $StackDir
        if ($Teardown) {
            & (Join-Path $ComponentsDir 'agents/codex/scripts/teardown-local.ps1') -TargetDir $Target -Endpoint $Conf['agent_audit.elasticsearch.url']
        }
        else {
            & (Join-Path $ComponentsDir 'agents/codex/scripts/setup-audit.ps1') `
            ...
            & (Join-Path $ComponentsDir 'agents/codex/scripts/render-mcp.ps1') -TargetDir $Target -Endpoint $Conf['agent_audit.elasticsearch.url']
            & (Join-Path $ComponentsDir 'agents/codex/scripts/link-auth.ps1') -CodexHome (Join-Path $Target '.codex') -Endpoint $Conf['agent_audit.elasticsearch.url']
        }
    }
    'project' {
        if (-not $Target) {
    ...
    'managed' {
        ...
        if ($Teardown) {
            & (Join-Path $ComponentsDir 'agents/codex/scripts/teardown-managed.ps1') -WithHooks
        }
        else {
