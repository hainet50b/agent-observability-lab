...
split by reader: the backend control plane (targets, the single all-assets `elastic` group,
dependency edges) lives in the shared espalier project at
`../../backends/elastic/espalier.toml` — `scripts/provision.{sh,ps1}` just
runs `espalier apply --group elastic` in the stack's compose network —
...
```
claude-elastic/
├─ docker-compose.yml                     # thin composition: `include:`s the Elastic service fragments
...
`agent-config` CLI's `place` (`cargo run` against `../../agent-config/`). The
Elastic backend family (`../../backends/elastic/`) is self-contained: each service
fragment (`elasticsearch/`, `kibana/`, `apm-server/`) owns its service definition,
config, and the assets that service consumes (the `traces-apm@custom` /
`logs-apm.app@custom` ingest pipelines under `elasticsearch/`; the Claude Code data
views and saved searches under `kibana/claude/`), and the espalier project
(`espalier.toml`) plus `provision.{sh,ps1}` that load them sit beside the fragments.
The agent-side content (`../../agents/claude/`) owns only agent-runtime config — the
