...
switch ($Scope) {
    'local' {
        ...
        $Target = $StackDir
        if ($Teardown) {
            & (Join-Path $ComponentsDir 'agents/claude/scripts/teardown-local.ps1') -TargetDir $Target -Endpoint $OtlpEndpoint
        }
        else {
            & (Join-Path $ComponentsDir 'agents/claude/scripts/setup-telemetry.ps1') -TargetDir $Target -OtlpEndpoint $OtlpEndpoint -ApiKey $OtlpApiKey
            & (Join-Path $ComponentsDir 'agents/claude/scripts/render-mcp.ps1') -TargetDir $Target -Endpoint $OtlpEndpoint
        }
    }
    'project' {
        if (-not $Target) {
    ...
    'managed' {
        if ($Teardown) {
            ...
            & (Join-Path $ComponentsDir 'agents/claude/scripts/teardown-managed.ps1') @TeardownArgs
        }
        else {
