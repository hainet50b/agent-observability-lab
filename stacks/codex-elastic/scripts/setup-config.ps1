...
switch ($Scope) {
    'local' {
        ...
        $Target = $StackDir
        if ($Teardown) {
            & (Join-Path $ComponentsDir 'agents/codex/scripts/teardown-local.ps1') -TargetDir $Target -Endpoint $OtlpEndpoint
        }
        else {
            & (Join-Path $ComponentsDir 'agents/codex/scripts/setup-telemetry.ps1') -TargetDir $Target -OtlpEndpoint $OtlpEndpoint -ApiKey $OtlpApiKey
            & (Join-Path $ComponentsDir 'agents/codex/scripts/render-mcp.ps1') -TargetDir $Target -Endpoint $OtlpEndpoint
            & (Join-Path $ComponentsDir 'agents/codex/scripts/link-auth.ps1') -CodexHome (Join-Path $Target '.codex') -Endpoint $OtlpEndpoint
        }
    }
    'project' {
        if (-not $Target) {
    ...
    'managed' {
        if ($Teardown) {
            ...
            & (Join-Path $ComponentsDir 'agents/codex/scripts/teardown-managed.ps1') @TeardownArgs
        }
        else {
            & (Join-Path $ComponentsDir 'agents/codex/scripts/setup-managed.ps1') `
