...
local)
  ...
  if [ "$teardown" -eq 1 ]; then
    "$components_dir/agents/codex/scripts/teardown-local.sh" "$target" "$otlp_endpoint"
  else
    "$components_dir/agents/codex/scripts/setup-telemetry.sh" "$target" "$otlp_endpoint" "$OTLP_API_KEY"
    "$components_dir/agents/codex/scripts/render-mcp.sh" "$target" "$otlp_endpoint"
    "$components_dir/agents/codex/scripts/link-auth.sh" "$target/.codex" "$otlp_endpoint"
  fi
  ;;
project)
  [ -n "$target" ] || {
...
managed)
  if [ "$teardown" -eq 1 ]; then
    ...
    "$components_dir/agents/codex/scripts/teardown-managed.sh" $teardown_args
  else
    "$components_dir/agents/codex/scripts/setup-managed.sh" \
