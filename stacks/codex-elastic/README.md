...
control plane (targets, the single all-assets `elastic` group, dependency edges) lives in the shared espalier
project at `../../backends/elastic/espalier.toml` — `scripts/provision.{sh,ps1}` just
runs `espalier apply --group elastic` in the stack's compose network — and the
...
modelled on `backends/elastic/kibana/claude/`. (Prompt / tool-call **audit**
...
(`elasticsearch/`, `kibana/`, `apm-server/`) owns its service definition, config, and
the assets that service consumes (the `traces-apm@custom` /
...
espalier project plus `provision.{sh,ps1}` sit beside the fragments. The
agent-side content (`../../agents/codex/`) owns
