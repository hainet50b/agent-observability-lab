...
> [`codex-cli-elastic`](../codex-cli-elastic/): a direct hook → Elasticsearch write with
...
sibling [`codex-cli-elastic`](../codex-cli-elastic/) stack instead.
...
`stacks/codex-cli-elastic-audit/.codex/` (gitignored) carrying the audit config —
...
Audit records are **agent-cross-cutting**: the AI agent (Codex CLI / Claude Code / …) is a
...
appliers that load them. The Codex CLI agent component (`../../components/agents/codex-cli/`)
