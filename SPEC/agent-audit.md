...
The audit data streams are provisioned (strict templates + data streams) and viewed (cross-agent Kibana data views / saved searches) by the **Elastic backend family** (`backends/elastic/`); audit is a direct hook → Elasticsearch write that never uses the OTLP / APM path, so the `<agent>-elastic-audit` stacks (e.g. `codex-elastic-audit`) compose only the `elasticsearch` + `kibana` fragments — **no APM Server** — while remaining a separate compose project with separate volumes and a separate agent home from the telemetry stacks (whose `.codex` carries `config.toml` — the inline `[hooks]` registrations plus the Elasticsearch MCP — and `agent-audit.conf`, but no `[otel]` config). The split keeps each stack's load-bearing set legible; the two stacks are alternatives, not run simultaneously.
