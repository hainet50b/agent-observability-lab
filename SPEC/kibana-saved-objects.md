...
| **API Errors** (`message` is one of `claude_code.api_error` / `claude_code.api_retries_exhausted` — a `phrases` pill) | `message` (error vs retries-exhausted), `model`, `error`, `status_code`; `numeric_labels` `attempt`, `duration_ms` (`api_error` rows only), `total_attempts`, `total_retry_duration_ms` (`api_retries_exhausted` rows only — gaps expected) | `request_id`; `internal_error` events (envelope + `error_name: "Error"`, nothing else). The exhausted row duplicates its paired `api_error` but stays in view — it is the distinct signal future alerting hangs off |
