...
| **API Errors** (`api_error`) | `model`, `error`, `status_code`; `numeric_labels` `attempt`, `duration_ms` | `request_id`; `internal_error` events (envelope + `error_name: "Error"`, nothing else — their substance is the paired 500-class `api_error`) |
| **API Retries Exhausted** (`api_retries_exhausted`) | `model`, `error`, `status_code`; `numeric_labels` `total_attempts`, `total_retry_duration_ms` | — content duplicates the paired terminal `api_error`, but "retries fully exhausted" is the distinct signal future alerting hangs off, so it gets its own view; empty in normal operation |
