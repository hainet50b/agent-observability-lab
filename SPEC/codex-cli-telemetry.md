# Codex CLI telemetry reference (codex-cli-elastic)

What OpenAI Codex CLI actually emits into the `codex-cli-elastic` stack, captured
...
- **Service name is `codex_cli_rs`, not `codex-cli`.** Codex emits the Rust
  ...
  The lab's human-facing names stay `codex-cli` / `codex-cli-elastic`; everything
