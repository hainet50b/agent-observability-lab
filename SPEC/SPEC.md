...
|---|---|---|---|---|---|---|
| `claude-code-elastic` | Claude Code | telemetry | `elastic` | direct | `telemetry.apm_server.endpoint` `:8200` | ES `9200`, Kibana `5601`, APM `8200` |
| `codex-cli-elastic` | Codex CLI | telemetry | `elastic` | direct | `telemetry.apm_server.endpoint` `:8200` | ES `9200`, Kibana `5601`, APM `8200` |
| `claude-code-otelcol-elastic` | Claude Code | telemetry | `elastic` | `otelcol-sidecar` | `telemetry.otel_collector.endpoint` `:4318` | + Collector `4317`/`4318` |
| `claude-code-elastic-audit` | Claude Code | audit | `elastic-audit` | direct | `agent_audit.elasticsearch.url` `:9200` | ES `9200`, Kibana `5601` (no APM) |
| `codex-cli-elastic-audit` | Codex CLI | audit | `elastic-audit` | direct | `agent_audit.elasticsearch.url` `:9200` | ES `9200`, Kibana `5601` (no APM) |
...
- [`codex-cli-managed-config.md`](codex-cli-managed-config.md) — Codex CLI's admin-managed configuration: the three layers (`managed_config.toml` defaults, `requirements.toml` enforcement, cloud requirements), exact paths/precedence per platform, the unified hook schema across `hooks.json` / inline `config.toml` / `requirements.toml` (`managed_dir`, `allow_managed_hooks_only`), and why `[otel]` is defaulted-not-enforced and Windows managed defaults are a weak boundary. The deploy-only placement model it shares with Claude is owned by [`config-deployment.md`](config-deployment.md).
