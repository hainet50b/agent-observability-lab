...
- **Identity & PII:** `service.name` is `claude-code` (the smoke probe uses `cce-smoke-test`); `session.id` is promoted to a top-level field. `labels.user_email` / `labels.user_id` and (when `OTEL_LOG_USER_PROMPTS=1`) `labels.prompt` are PII — but they live in the **ingested documents** regardless of any view, so the sensitivity is about who can query Elasticsearch / open Kibana and the "don't send confidential sessions / don't commit captured telemetry" invariants, **not** about which columns a saved search lists (a saved object stores column *names*, not data). Choosing saved-search columns is a usefulness call, not a privacy control.
