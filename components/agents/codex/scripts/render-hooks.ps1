...
if (-not (Test-Path -LiteralPath (Join-Path $HooksSrc 'agent-audit.ps1') -PathType Leaf)) {
    Write-Error "FAIL: hook not found: $(Join-Path $HooksSrc 'agent-audit.ps1')"
    exit 1
}
...
Add-CpSection 'hook' 'codex' $Endpoint $block $config '[[hooks.UserPromptSubmit]]'
Set-CpSelfIgnore 'codex' $Endpoint (Join-Path $targetAbs '.codex')
