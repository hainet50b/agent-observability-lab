...
try {
    ...
    Set-CpFile 'agent-audit' 'codex' $Marker $tmp $config
    if ($SealRecipientsSrc) {
        Set-CpFile 'agent-audit' 'codex' $Marker $SealRecipientsSrc $placedCert
    }
    Set-CpSelfIgnore 'codex' $Marker (Join-Path $TargetDir '.codex')
}
finally {
