...
try {
    ...
    Set-CpFile 'agent-audit' 'claude' $Marker $tmp $config
    if ($SealRecipientsSrc) {
        Set-CpFile 'agent-audit' 'claude' $Marker $SealRecipientsSrc $placedCert
    }
    Set-CpSelfIgnore 'claude' $Marker (Join-Path $TargetDir '.claude')
}
finally {
