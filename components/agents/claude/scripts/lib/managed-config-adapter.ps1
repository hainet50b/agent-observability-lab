...
function Get-McManifest {
    ...
    [pscustomobject]@{ Key = 'managed-settings'; Source = $Sources[0]; Target = "$root${sep}managed-settings.d${sep}10-$($script:McTool).json" }
    if ($script:McWithHooks) { Get-McHookManifestItem "$root${sep}hooks${sep}$($script:McTool)" (Get-McHookFlavor $Os) }
}
