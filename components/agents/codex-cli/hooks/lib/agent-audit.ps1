...
function Assert-ConfigArg($argList) {
    ...
    for ($i = 0; $i -lt $argList.Count; $i++) {
        if ($argList[$i] -eq '-Config' -and $i + 1 -lt $argList.Count) { $script:configFile = $argList[$i + 1] }
    }
    if (-not $configFile) {
...
function Read-HookPayload {
    try {
        ...
        if ($stream -eq 'user_prompt') {
            $script:promptText = $rawObj.prompt
            $script:promptLen = if ($null -ne $promptText) { ([string]$promptText).Length } else { 0 }
        }
        else {
